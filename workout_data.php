<?php
	...
	function get_package_seats($date,$package_id){
		...
		return mysqli_num_rows(select("SELECT bookingid  FROM `booking` WHERE `booking_for`  BETWEEN '$date 00:00:00' AND '$date 23:59:59' and package_id = '$package_id' and movie_id='".$_REQUEST['workoutid']."'  "));
	}
	...
	<div class="all-comments-info" id="booking_form">
		...
		<div class="agile-info-wthree-box">
			...
			<div class="col-md-6 form-info">
				<select name="package_id" id="package_id"  class="form-control">
					...
					<?php while($row1 = mysqli_fetch_array($package_data2)) { extract($row1) ?>
						...
						if($res2 > 0) : ?>
						<option data-package_limit = "<?=$res2?>" value="<?=$package_id?>" <?php if(isset($_POST['package_id'])){ if( $_POST['package_id']==$package_id)  { echo "selected";  } } ?>><?=strtoupper($package_name)?> - Rs. <?=$package_rate?> /-</option>
						<?php endif;?>
					<?php } ?>
				</select>
			</div>
			...
			<input type="submit" id="book_ticket" value="Book" data-seats_available="0" data-workoutid="<?=$_REQUEST['workoutid']?> ">
			<div class="clearfix"> </div>
			<br/>
			<br/>
